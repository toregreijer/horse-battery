# -*- coding: utf-8 -*-
### Automatically generated by repyhelper.py ### C:\Dropbox\uni\y1p2\dist\lab\demokit\geoip_client.repy
...
def geoip_init_client(url=["http://geoip.cs.washington.edu:12679", "http://betaseattleclearinghouse.poly.edu:12679"]):
  """
  <Purpose>
...
def geoip_record_by_addr(addr, timeout=5):
  ...
  for geoip_client in geoip_clientlist:
    try:
      return geoip_client.send_request("record_by_addr", (addr,), timeout)
    except:
      pass
  else:
...
def geoip_record_by_name(name, timeout=5):
  ...
  for geoip_client in geoip_clientlist:
    try:
      return geoip_client.send_request("record_by_name", (name,), timeout)
    except:
      pass
  else:
    raise Exception("Unable to contact the geoip server.")
